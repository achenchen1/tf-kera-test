...
for key, value in folder_dict.items():
    folder_iter[key] = len(os.listdir(os.path.join("images", value)))
...
for f in os.listdir():
    if f[-3:] == "png":
        ...
        selection = input()
        if folder_dict[selection] in os.listdir("images"):
            os.rename(f, os.path.join("images", folder_dict[selection], folder_dict[selection]+"_"+str(folder_iter[selection])+".png"))
        else:
            print("BAD")
            break
    else:
